...
class SharedPreferencesDataStoreTest {
    ...
    @Test
    fun `SettingsRepository preferenceDataStore targets the repository's own store`() {
        val repo = SettingsRepository(prefs)
        repo.preferenceDataStore().putBoolean("devModeEnabled", true)
        repo.devModeEnabled.shouldBeTrue()
    }
}
